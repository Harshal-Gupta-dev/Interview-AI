...
export default function Interview() {
    ...
    const handlePdfDownload = async () => {
        ...
        setPdfLoading(true);
        try {
            // Passes the pure upper scope string variable down to your Axios hook safely
    ...
    // 🟢 SAFE: Checks if report exists before fetching score data
    const currentScore = report?.matchScore || 0;
    const strokeDashoffset = circumference - (currentScore / 100) * circumference;

    // Severity color utility mapper function
    ...
    return (
        <div className="h-screen w-screen bg-[#0d0e12] text-white flex overflow-hidden antialiased font-sans">
            {pdfLoading && (
                <div className="fixed inset-0 bg-black/75 backdrop-blur-md z-50 flex flex-col items-center justify-center space-y-4">
                    ...
                    <div className="text-center space-y-1">
                        <p className="text-white font-semibold text-base tracking-wide animate-pulse">
            ...
            <div className="w-64 bg-[#111217] border-r border-gray-800/60 flex flex-col justify-between flex-shrink-0">
                <div className="p-5 space-y-6">
                    <div className="text-xs font-bold text-gray-500 tracking-widest uppercase px-2">Generated Report</div>
                    <nav className="space-y-1.5">
                        <button
                            onClick={() => { setActiveTab('technical'); setExpandedIndex(0); }}
                            className={`w-full flex items-center space-x-3 px-4 py-3 rounded-xl text-sm font-semibold tracking-wide transition-all duration-150 ${activeTab === 'technical' ? 'bg-pink-500/10 text-[#ec4899] border-l-4 border-[#ec4899] rounded-l-none' : 'text-gray-400 hover:text-gray-200 hover:bg-[#1a1c23]'}`}
                        >
                            <span>{`</>`}</span>
                            <span>Technical Questions</span>
                        </button>
                        <button
                            onClick={() => { setActiveTab('behavioral'); setExpandedIndex(0); }}
                            className={`w-full flex items-center space-x-3 px-4 py-3 rounded-xl text-sm font-semibold tracking-wide transition-all duration-150 ${activeTab === 'behavioral' ? 'bg-pink-500/10 text-[#ec4899] border-l-4 border-[#ec4899] rounded-l-none' : 'text-gray-400 hover:text-gray-200 hover:bg-[#1a1c23]'}`}
                        >
                            <span>💬</span>
                            <span>Behavioral Questions</span>
                        </button>
                        <button
                            onClick={() => { setActiveTab('roadmap'); setExpandedIndex(0); }}
                            className={`w-full flex items-center space-x-3 px-4 py-3 rounded-xl text-sm font-semibold tracking-wide transition-all duration-150 ${activeTab === 'roadmap' ? 'bg-pink-500/10 text-[#ec4899] border-l-4 border-[#ec4899] rounded-l-none' : 'text-gray-400 hover:text-gray-200 hover:bg-[#1a1c23]'}`}
                        >
                            <span>📍</span>
                            <span>Road Map</span>
                        </button>
                ...
                <div>
                    <div className="px-5 pb-2">
                        <button
                            // 🟢 4. FIXED: Clean empty execution arrow pointing to our wrapper handler
                            onClick={() => handlePdfDownload()}
                            disabled={pdfLoading}
                            className="w-full flex items-center justify-center space-x-2.5 px-4 py-3 bg-[#ec4899] hover:bg-[#db2777] disabled:bg-gray-800 text-white text-sm font-semibold tracking-wide rounded-xl shadow-[0_4px_20px_rgba(236,72,153,0.15)] hover:shadow-[0_4px_25px_rgba(236,72,153,0.3)] border border-pink-400/20 active:scale-[0.98] transition-all duration-150"
                        >
                            {/* Modern AI Document Generation Spark Icon */}
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4 animate-pulse">
                                ...
                                <path d="M9 15h6" />
                                <path d="M9 11h3" />
                                {/* Embedded AI Magic Spark Node */}
                                <path d="M17 14l1.5 1.5L20 14l-1.5-1.5z" fill="currentColor" stroke="none" />
                                <path d="M18.5 12.5l.5.5.5-.5-.5-.5z" fill="currentColor" stroke="none" />
                            </svg>
                            <span>Generate PDF</span>
                        </button>
                    </div>
                    <div className="p-4 border-t border-gray-800/40 text-[11px] font-mono text-gray-600 text-center tracking-wide">
                        Dashboard View v1.0
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 bg-[#0d0e12] p-6 md:p-8 flex flex-col overflow-y-auto h-full">
                ...
                <div className="mb-6 flex items-center justify-between">
                    ...
                    <span className="bg-[#14161d] border border-gray-800 text-[11px] px-3 py-1 font-mono text-gray-400 rounded-full">
                ...
                <div className="space-y-4 flex-1">
                    ...
                    {/* 🟢 SAFE: Optional chaining added directly to report wrapper target */}
                    {activeTab === 'technical' && report?.technicalQuestion?.map((q, idx) => (
                        <div key={idx} className="bg-[#14161d] border border-gray-800/60 rounded-xl overflow-hidden transition-all duration-200">
                            <button
                                ...
                                className="w-full text-left p-4 flex items-start justify-between space-x-4 hover:bg-[#1a1d26] transition-colors"
                            >
                                <div className="flex items-start space-x-3">
                                    <span className="text-xs font-bold tracking-wider font-mono text-[#ec4899] bg-pink-500/10 px-2 py-0.5 rounded mt-0.5">Q{idx + 1}</span>
                                    <span className="text-sm font-semibold text-gray-100 leading-relaxed">{q.question}</span>
                                </div>
                                <span className="text-gray-500 text-xs transform transition-transform duration-150 mt-1">
                                    {expandedIndex === idx ? '▲' : '▼'}
                                </span>
                            </button>
                            ...
                            {expandedIndex === idx && (
                                <div className="p-4 bg-[#181a22] border-t border-gray-800/40 space-y-3.5 text-xs animate-fadeIn">
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <p className="text-gray-300 leading-relaxed font-medium bg-[#13141a] border border-gray-800/60 rounded-lg p-3">{q.answer}</p>
                                    </div>
                                </div>
                            )}
                        </div>
                    ...
                    {activeTab === 'behavioral' && report?.behavioralQuestion?.map((q, idx) => (
                        <div key={idx} className="bg-[#14161d] border border-gray-800/60 rounded-xl overflow-hidden transition-all duration-200">
                            ...
                            >
                                <div className="flex items-start space-x-3">
                                    <span className="text-xs font-bold tracking-wider font-mono text-[#ec4899] bg-pink-500/10 px-2 py-0.5 rounded mt-0.5">B{idx + 1}</span>
                                    <span className="text-sm font-semibold text-gray-100 leading-relaxed">{q.question}</span>
                                </div>
                                <span className="text-gray-500 text-xs transform transition-transform duration-150 mt-1">
                                    {expandedIndex === idx ? '▲' : '▼'}
                    ...
                    {activeTab === 'roadmap' && report?.preparationPlan?.map((p, idx) => (
                        <div key={idx} className="bg-[#14161d] border border-gray-800/60 rounded-xl p-5 space-y-4 shadow-xl">
                            <div className="flex items-center justify-between border-b border-gray-800/40 pb-3">
                                ...
                                <span className="text-[10px] text-gray-500 font-mono font-medium"> Sprint Module 0{idx + 1}</span>
                            </div>
                            <ul className="space-y-2.5">
                                {p.tasks?.map((task, tIdx) => (
                                    <li key={tIdx} className="flex items-start space-x-3 text-xs font-medium text-gray-400 leading-relaxed">
                                        <span className="text-[#ec4899] mt-0.5 select-none">✔</span>
            ...
            <div className="w-80 bg-[#111217] border-l border-gray-800/60 p-6 flex flex-col space-y-6 flex-shrink-0 h-full overflow-y-auto">
                ...
                <div className="bg-[#14161d] border border-gray-800/60 rounded-xl p-5 flex flex-col items-center text-center shadow-lg">
                    ...
                    {/* Circular Dynamic Progress Render Gauge */}
                    <div className="relative flex items-center justify-center mb-3">
                        ...
                        <div className="absolute text-xl font-extrabold font-mono text-white flex flex-col items-center">
                            {/* 🟢 SAFE: Optional chaining safely wraps layout state */}
                ...
                <div
                    className="space-y-2.5 max-h-[55vh] overflow-y-auto pr-2"
                    style={{
                        scrollbarWidth: 'thin',
                ...
                >
                    ...
                    {/* 🟢 SAFE: Optional chaining safely monitors sidebar array data feeds */}
                    {/* 🟢 FIXED: Removed the activeTab === 'skillGaps' check so it displays normally! */}
                    {report?.skillGaps?.map((gap, idx) => (
                        <div key={idx} className="bg-[#14161d] border border-gray-800/40 rounded-xl p-3.5 space-y-2 hover:border-gray-800 transition-all shadow-md">
                            <div className="text-xs font-bold text-gray-200 leading-relaxed">
                                {gap.skill}
                            </div>
                            <div className="flex justify-end">
                                <span className={`text-[10px] font-mono font-bold px-2 py-0.5 rounded-md border tracking-wider uppercase ${getSeverityStyle(gap.severity)}`}>
